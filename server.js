...
union() {
    ...
    translate([${-(d.xPosN || 0)}, ${d.yPosN || 0}, -0.3]) mirror([1, 0, 0])
    linear_extrude(height=0.8) 
    ...
    try {
        ...
        exec(`openscad -o "${stlPath}" "${scadPath}"`, async (error) => {
            ...
            try {
                ...
                await supabase.storage.from(bucket).upload(`final/${id}.stl`, fileBuffer, { contentType: 'model/stl', upsert: true });
...
app.listen(PORT, '0.0.0.0', () => console.log("Server Prod OK"));